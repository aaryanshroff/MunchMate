...
function RestaurantPage() {
  ...
  useEffect(() => {
    async function fetchRestaurant() {
      try {
        setIsLoading(true);
        const response = await axios.get(`/api/restaurants/${restaurant_id}`);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        setMyReview({ uid: currentUserId, rating, review_text: reviewText, created_at: new Date().toISOString() });
        ...
        <>
            ...
            <>
              {myReview ? (
                <div className="card p-3 mb-3">
                  <h5>Your Review (Posted on {new Date(myReview.created_at).toLocaleString()}):</h5>
                  <p>
                    <strong>Rating:</strong> {myReview.rating}
                  </p>
                  <p>
                    <strong>Review:</strong> {myReview.review_text}
                  </p>
                </div>
              ) : (
          ...
          <div className="mb-3">
            ...
            <select
              ...
              className="form-select form-select-sm"
              value={reviewsLimit}
              onChange={(e) => setReviewsLimit(parseInt(e.target.value, 10))}
              style={{ width: "150px" }}
            >
          ...
          {restaurantReviews.length === 0 && !reviewsLoading ? (
            ...
            restaurantReviews.map((review, index) => (
              <div key={index} className="card mb-3 p-3">
                ...
                <p>
                  <strong>Reviewed at:</strong> {new Date(review.created_at).toLocaleString()}
                </p>
              </div>
            ))
          )}
